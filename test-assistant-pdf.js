...
async function testPdfProcessing() {
  ...
  try {
    ...
    const assistant = await openai.beta.assistants.create({
      ...
      model: "gpt-4o",
      instructions: "Extract all text from PDF documents accurately, maintaining structure and formatting.",
    ...
    try {
      ...
      await openai.beta.threads.messages.create(thread.id, {
        role: 'user',
        content: 'Please extract all text from this PDF document. Maintain all formatting, paragraphs, and structure.',
        attachments: [{ 
          file_id: file.id,
          tools: [{ type: "file_search" }]
        }],
      ...
      if (runStatus.status !== 'completed') {
        throw new Error(`PDF processing failed or timed out: ${runStatus.status}`);
      }
