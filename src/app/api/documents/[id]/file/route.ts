...
export async function GET(req: Request, context: { params: { id: string } }) {
  try {
    ...
    if (document.type === "pdf") {
      ...
      const pdfFilePath = path.join(tempDir, `${document.id}-${document.fileName || "document.pdf"}`);
      ...
      if (fs.existsSync(pdfFilePath)) {
        ...
        headers.set('Content-Disposition', `inline; filename="${document.fileName || 'document.pdf'}"`);
      ...
      return NextResponse.json({ 
        ...
        message: "The PDF file is not available for direct download. Please contact support if this issue persists."
      }, { 
        status: 404 
      });
    }
