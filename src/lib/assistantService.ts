...
export async function getPdfAssistant() {
  try {
    ...
    if (PDF_ASSISTANT_ID) {
      try {
        const assistant =
          await openai.beta.assistants.retrieve(PDF_ASSISTANT_ID);
        console.log(`Using existing PDF assistant: ${assistant.id}`);
        return assistant;
      } catch (retrieveError) {
        console.warn(
          `Could not retrieve assistant with ID ${PDF_ASSISTANT_ID}, creating new one`,
        );
      }
    }
    ...
    const assistant = await openai.beta.assistants.create({
      name: "PDF Processor",
      description:
        "Assistant specialized in extracting and processing text from PDFs",
 ...
 */
export async function processPdfWithAssistant(
  document: DocumentModel,
): Promise<string> {
  try {
    console.log(
      `Processing PDF document ${document.id} with OpenAI Assistant API`,
    );
    ...
    if (document._buffer) {
      ...
      throw new Error(
        `No content source available for document ${document.id}`,
      );
    }
    ...
    try {
      ...
      await openai.beta.threads.messages.create(thread.id, {
        role: "user",
        content:
          "Please extract all text from this PDF document. Maintain all formatting, paragraphs, tables, and structure.",
        attachments: [
          {
            file_id: file.id,
            tools: [{ type: "file_search" }],
      ...
      if (runStatus.status !== "completed") {
        throw new Error(
          `Assistant processing timed out or failed with status: ${runStatus.status}`,
        );
      }
...
export async function cancelAllActiveRuns() {
  try {
    for (const [documentId, threadId] of Object.entries(activeThreads)) {
      try {
        console.log(
          `Cancelling runs for document ${documentId} on thread ${threadId}`,
        );
        ...
        console.error(
          `Error cancelling runs for document ${documentId}:`,
          error,
        );
      }
    }
